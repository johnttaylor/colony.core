...
///////////////////////////
Cpl::TShell::Command::Result_T Dm::execute( Cpl::TShell::Context_& context, Cpl::Text::Tokenizer::TextBlock& tokens, const char* rawCmdString, Cpl::Io::Output& outfd ) noexcept
{
	...
	// READ Sub-command
	else if ( strcmp( tokens.getParameter( 1 ), "read" ) == 0 )
	{
	...
	// WRITE sub-command
	else if ( strcmp( tokens.getParameter( 1 ), "write" ) == 0 )
	{
		// Find the start of the JSON object
		const char* cmd    = Cpl::Text::stripSpace( rawCmdString );
		const char* subcmd = Cpl::Text::stripSpace( Cpl::Text::stripNotSpace( cmd ) );
