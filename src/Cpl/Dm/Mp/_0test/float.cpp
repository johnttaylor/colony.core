...
TEST_CASE( "float-get", "[float-get]" )
{
	...
	REQUIRE( strcmp( mpType, "Cpl::Dm::Mp::Float" ) == 0 );
...
TEST_CASE( "float-toJSON" )
{
	CPL_SYSTEM_TRACE_SCOPE( SECT_, "FLOAT-TOJSON test" );
	Cpl::System::Shutdown_TS::clearAndUseCounter();
	...
	SECTION( "Value" )
	{
		uint16_t seqnum = mp_apple_.write( -127e10F, ModelPoint::eUNLOCK );
		mp_apple_.toJSON( string, MAX_STR_LENG, truncated );
		CPL_SYSTEM_TRACE_MSG( SECT_, ( "toJSON: [%s])", string ) );
...
///////////////////////////////////////////////////////////////////////////////
TEST_CASE( "float-fromJSON" )
{
	CPL_SYSTEM_TRACE_SCOPE( SECT_, "FLOAT-FROMJSON test" );
	Cpl::System::Shutdown_TS::clearAndUseCounter();
	...
	SECTION( "Write value - error cases" )
	{
		...
		REQUIRE( ModelPoint::IS_VALID( valid ) );
		REQUIRE_FLOAT_EQUAL( value, 1.23F );
		REQUIRE( errorMsg == "noerror" );
		...
		valid = mp_apple_.read( value );
		REQUIRE( ModelPoint::IS_VALID( valid ) );
		REQUIRE_FLOAT_EQUAL( value, 2.2e10F );
		REQUIRE( errorMsg == "noerror" );
	...
	SECTION( "lock..." )
	{
		...
		REQUIRE_FLOAT_EQUAL( value, 43.21F );
		...
		valid = mp_apple_.read( value );
		REQUIRE( ModelPoint::IS_VALID( valid ) == true );
		REQUIRE_FLOAT_EQUAL( value, 21.0F );
