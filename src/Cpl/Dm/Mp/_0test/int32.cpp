...
TEST_CASE( "int32-get", "[int32-get]" )
{
    ...
    REQUIRE( strcmp( mpType, "Cpl::Dm::Mp::Int32-dec" ) == 0 );

    mpType = mp_orange_.getTypeAsText();
    CPL_SYSTEM_TRACE_MSG( SECT_, ( "typeText: [%s])", mpType ) );
    REQUIRE( strcmp( mpType, "Cpl::Dm::Mp::Int32-hex" ) == 0 );
...
TEST_CASE( "int32-toJSON" )
{
    CPL_SYSTEM_TRACE_SCOPE( SECT_, "INT32-TOJSON test" );
    Cpl::System::Shutdown_TS::clearAndUseCounter();
...
///////////////////////////////////////////////////////////////////////////////
TEST_CASE( "int32-fromJSON" )
{
    CPL_SYSTEM_TRACE_SCOPE( SECT_, "INT32-FROMJSON test" );
    Cpl::System::Shutdown_TS::clearAndUseCounter();
    ...
    SECTION( "Write value - error cases" )
    {
        ...
        errorMsg = "noerror";
        json     = "{name:\"APPLE\", val:8589934591}";  // Exceed range of the int32_t
        result   = modelDb_.fromJSON( json, &errorMsg );
        CPL_SYSTEM_TRACE_MSG( SECT_, ( "fromSJON errorMsg=[%s])", errorMsg.getString() ) );
        REQUIRE( result == true );
        valid = mp_apple_.read( value );
