...
static bool are_record_values_match( MyRecord& record, const char* mp1Value, const char* mp2Value, const char* mp3Value )
{
    ...
    record.m_mp1.read( value, mpState ); if ( Cpl::Dm::ModelPoint::IS_VALID( mpState ) == false || value != mp1Value ) return false;
    record.m_mp2.read( value, mpState ); if ( Cpl::Dm::ModelPoint::IS_VALID( mpState ) == false || value != mp2Value ) return false;
    record.m_mp3.read( value, mpState ); if ( Cpl::Dm::ModelPoint::IS_VALID( mpState ) == false || value != mp3Value ) return false;
    return true;
}
