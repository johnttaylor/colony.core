...
///////////////////////////////
Listener::Listener( Cpl::Log::Api& eventLogger )
:m_fd(-1),
...
void Listener::listen()
	...
	if ( setsockopt(m_fd, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one)) ) 
	    {
		m_logger.warning( "Cpl::Io::Socket::Posix::Listener: Failed setsockopt SO_REUSEADDR" );
	    }
    ...
    while( retry )
        ...
        m_logger.warning( "Cpl::Io::Socket::Posix::Listener:: Listening on Port=%d. Bind error - retrying...", m_socket ); 
    ...
    for(;;)
        ...
        if ( setsockopt( newfd, SOL_SOCKET, SO_KEEPALIVE, (char*)&bOptVal, bOptLen) < 0 )
            {
			int sockerr = errno;
            m_logger.warning( "Cpl::Io::Socket::Posix::Listener:: start() - Failed enable SO_KEEPALIVE. errCode=(%X) %s.", sockerr, strerror(sockerr) );
            }
