...
public:
    ...
    const char* getUsage() const noexcept    { return CPLTSHELLDACMD_USAGE_INC_; }

    /// See Cpl::TShell::Dac::Command
    const char* getHelp() const noexcept    { return CPLTSHELLDACMD_DETAIL_INC_; }
    
     
public:
    /// Constructor
    Inc( Cpl::Container::Map<Cpl::TShell::Dac::Command>& commandList ) noexcept;

    /// Constructor.  Used to create a static instance of the command
    Inc( Cpl::Container::Map<Cpl::TShell::Dac::Command>& commandList, const char* ignoreThisParameter_onlyUsedWhenCreatingAStaticInstance ) noexcept;
