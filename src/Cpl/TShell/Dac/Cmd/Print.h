...
public:
    ...
    const char* getUsage() const noexcept    { return CPLTSHELLDACMD_USAGE_PRINT_; }

    /// See Cpl::TShell::Dac::Command
    const char* getHelp() const noexcept    { return CPLTSHELLDACMD_DETAIL_PRINT_; }
    
     
public:
    /// Constructor
    Print( Cpl::Container::Map<Cpl::TShell::Dac::Command>& commandList ) noexcept;

    /// Constructor.  Used to create a static instance of the command
    Print( Cpl::Container::Map<Cpl::TShell::Dac::Command>& commandList, const char* ignoreThisParameter_onlyUsedWhenCreatingAStaticInstance ) noexcept;
