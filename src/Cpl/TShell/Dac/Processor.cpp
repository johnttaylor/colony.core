...
bool Processor::start( Cpl::Io::Input& infd, Cpl::Io::Output& outfd ) noexcept
{
	...
	outfd.write( OPTION_CPL_TSHELL_DAC_PROCESSOR_GREETING );
	...
	for ( ;;)
	{
		...
		if ( !run )
		{
			outfd.write( OPTION_CPL_TSHELL_DAC_PROCESSOR_FAREWELL );
		...
		// Output the prompt
		outfd.write( OPTION_CPL_TSHELL_DAC_PROCESSOR_PROMPT );
