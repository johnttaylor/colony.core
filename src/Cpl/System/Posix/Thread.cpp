...
:m_runnable(runnable),
 ...
 m_threadHandle(0)
    ...
    if ( rc_init || rc_setdetach || rc_setstack || rc_schedpolicy || rc_schedparam )
        {
        eventLogger.warning( "Cpl::System::Posix::Thread. unexpected error when creating thread %s. Returns codes (all should be zero): attr_init=%d, setdetach=%d, setstack=%d, schedpolicy=%, schedparam=%d", name, rc_init, rc_setdetach, rc_setstack, rc_schedpolicy, rc_schedparam );
        }
...
void* Thread::entryPoint(void* data)
    ...
    // is terminated immediately without waiting to reach a "cancelation point")
    int oldCancelType;
