...
static SPool<struct1_T, NUM_POOL7>       pool7_s1;
static HPool<struct1_T>                  pool8_s1(NUM_POOL8);
...
TEST_CASE( "basic", "[basic]" )
    ...
    SPool<struct1_T, NUM_POOL1>   pool1_s1;
    HPool<struct1_T>              pool2_s1(NUM_POOL2);
...
//
    ...
    for(i=0; i<NUM_POOL1; i++)
        ...
        CPL_SYSTEM_TRACE_MSG( SECT_, ( "STACK, S1, Static: ptr=%p, aligment=%d, sizeof=%p", s1Ptr, alignment, sizeof(struct1_T) ));
        REQUIRE( alignment == 0 );
        }
    s1Ptr = (struct1_T*) pool1_s1.allocate( sizeof(struct1_T) );
    REQUIRE( s1Ptr == 0 );
...
//
    ...
    for(i=0; i<NUM_POOL2; i++)
        ...
        cache_s1[i] = s1Ptr;
        int alignment = (size_t)s1Ptr % ALIGMENT;
        CPL_SYSTEM_TRACE_MSG( SECT_, ( "STACK, S1, Heap: ptr=%p, aligment=%d, sizeof=%p", s1Ptr, alignment, sizeof(struct1_T) ));
        REQUIRE( alignment == 0 );
        }
    s1Ptr = (struct1_T*) pool2_s1.allocate( sizeof(struct1_T) );
    REQUIRE( s1Ptr == 0 );
...
//
    ...
    for(i=0; i<NUM_POOL3; i++)
        ...
        CPL_SYSTEM_TRACE_MSG( SECT_, ( "STACK, S2, Static: ptr=%p, aligment=%d, sizeof=%p", s2Ptr, alignment, sizeof(struct2_T) ));
        REQUIRE( alignment == 0 );
        }
    s2Ptr = (struct2_T*) pool3_s2.allocate( sizeof(struct2_T) );
    REQUIRE( s2Ptr == 0 );
...
//
    ...
    for(i=0; i<NUM_POOL4; i++)
        ...
        REQUIRE( s2Ptr != 0 );
        int alignment = (size_t)s2Ptr % ALIGMENT;
        CPL_SYSTEM_TRACE_MSG( SECT_, ( "HEAP, S2, Static: ptr=%p, aligment=%d, sizeof=%p", s2Ptr, alignment, sizeof(struct2_T) ));
        REQUIRE( alignment == 0 );
        }
    s2Ptr = (struct2_T*) pool4_s2.allocate( sizeof(struct2_T) );
    REQUIRE( s2Ptr == 0 );
...
//
    ...
    for(i=0; i<NUM_POOL5; i++)
        ...
        REQUIRE( s2Ptr != 0 );
        int alignment = (size_t)s2Ptr % ALIGMENT;
        CPL_SYSTEM_TRACE_MSG( SECT_, ( "STATIC, S2, Static: ptr=%p, aligment=%d, sizeof=%p", s2Ptr, alignment, sizeof(struct2_T) ));
        REQUIRE( alignment == 0 );
        }
    s2Ptr = (struct2_T*) pool5_s2.allocate( sizeof(struct2_T) );
    REQUIRE( s2Ptr == 0 );
...
//
    ...
    for(i=0; i<NUM_POOL6; i++)
        ...
        REQUIRE( s2Ptr != 0 );
        int alignment = (size_t)s2Ptr % ALIGMENT;
        CPL_SYSTEM_TRACE_MSG( SECT_, ( "STATIC, S2, Heap: ptr=%p, aligment=%d, sizeof=%p", s2Ptr, alignment, sizeof(struct2_T) ));
        REQUIRE( alignment == 0 );
        }
    s2Ptr = (struct2_T*) pool6_s2.allocate( sizeof(struct2_T) );
    REQUIRE( s2Ptr == 0 );
...
//
    ...
    for(i=0; i<NUM_POOL7; i++)
        ...
        REQUIRE( s1Ptr != 0 );
        int alignment = (size_t)s1Ptr % ALIGMENT;
        CPL_SYSTEM_TRACE_MSG( SECT_, ( "STATIC, S1, Static: ptr=%p, aligment=%d, sizeof=%p", s1Ptr, alignment, sizeof(struct1_T) ));
        REQUIRE( alignment == 0 );
        }
    s1Ptr = (struct1_T*) pool7_s1.allocate( sizeof(struct1_T) );
    REQUIRE( s1Ptr == 0 );
...
//
    ...
    for(i=0; i<NUM_POOL8; i++)
        ...
        cache_s1[i] = s1Ptr;
        int alignment = (size_t)s1Ptr % ALIGMENT;
        CPL_SYSTEM_TRACE_MSG( SECT_, ( "STATIC, S1, Heap: ptr=%p, aligment=%d, sizeof=%p", s1Ptr, alignment, sizeof(struct1_T) ));
        REQUIRE( alignment == 0 );
        }
    s1Ptr = (struct1_T*) pool8_s1.allocate( sizeof(struct1_T) );
    REQUIRE( s1Ptr == 0 );
