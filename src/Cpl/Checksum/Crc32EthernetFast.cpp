...
/// Table driven, aka fast CRC calculation
static uint32_t const table_[256] = 
    {
...
#if 0
// Helper method to reflect the bits around the mid-bit
static uint32_t reflect_( uint32_t data, uint8_t nBits )
    {
...
void Crc32EthernetFast::accumulate( void* bytes, unsigned numbytes ) throw()
    ...
    for(i=0; i<numbytes; i++,ptr++)
        {
        uint8_t temp = *ptr ^ (m_crc >> (24));
  		m_crc         = table_[temp] ^ (m_crc << 8);
        }
    }
