...
//////////////////////////////////
void Server::request( OpenDbMsg& msg )
    {
    CPL_SYSTEM_TRACE_MSG( SECT_, ("Server::request( OpenDbMsg )") );

    Request::OpenDbPayload& operation = msg.getPayload();
    openDB( operation.m_result, operation.m_buffer, operation.m_bufferMaxSize );
    
    msg.returnToSender();
    }


void Server::request( CloseDbMsg& msg )
    {
    CPL_SYSTEM_TRACE_MSG( SECT_, ("Server::request( CloseDbMsg )") );
...
void Server::request( ClearDbMsg& msg )
    {
    CPL_SYSTEM_TRACE_MSG( SECT_, ("Server::request( ClearDbMsg )") );
    Request::ClearDbPayload& operation = msg.getPayload();
...
//////////////////////////////////
void Server::request( ReadMsg& msg )
    {
    CPL_SYSTEM_TRACE_MSG( SECT_, ("Server::request( ReadMsg )") );
    Request::ReadPayload& operation = msg.getPayload();

    // Trap improper usage
    if ( !m_fdPtr )
        {
        Cpl::System::FatalError::logf( "Rte::Db::Chunk::Server::request( ReadMsg ) - db file NOT opened." );
        }
    ...
    if ( !m_fdPtr->read(&(operation.m_handlePtr->m_len), Rte::Db::eCLEN_SIZE, bytesRead) || (uint32_t)bytesRead != Rte::Db::eCLEN_SIZE )
        {
        CPL_SYSTEM_TRACE_MSG( SECT_, ("Server::request( ReadMsg ) - eEOF - failed to read Chunk Length (bytesRead=%d)", bytesRead) );
