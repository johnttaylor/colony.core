...
#   Target:     STM32F4xxx Micro-controllers with FreeRTOS/CMSIS V2
...
class ToolChain( base.ToolChain ):
    ...
    #--------------------------------------------------------------------------
    def __init__( self, exename, prjdir, build_variants, pkg_root, sdk_root, bsp_root, linker_script, default_variant='release', env_error=None ):
        ...
        # Define paths
        sdk_path = os.path.join( pkg_root, sdk_root )
        bsp_path = os.path.join( pkg_root, bsp_root )
        self._base_release.inc = self._base_release.inc + \
                ' -I' + sdk_path + r'\Drivers\STM32F4xx_HAL_Driver\Inc' + \
                ' -I' + sdk_path + r'\Drivers\STM32F4xx_HAL_Driver\Inc\Legacy' + \
                ' -I' + sdk_path + r'\Drivers\CMSIS\Device\ST\STM32F4xx\Include' + \
                ' -I' + sdk_path + r'\Middlewares\Third_Party\FreeRTOS\Source\Include' + \
                ' -I' + sdk_path + r'\Middlewares\Third_Party\FreeRTOS\Source\CMSIS_RTOS_V2' + \
                ' -I' + sdk_path + r'\Middlewares\Third_Party\FreeRTOS\Source\portable\GCC\ARM_CM4F' + \
                ' -I' + sdk_path + r'\Drivers\CMSIS\Include' + \
                ' -I' + bsp_path + \
                ' -I' + bsp_path + r'\Core\Inc'

        # 
        mcu                             = '-mcpu=cortex-m4 -mfpu=fpv4-sp-d16 -mfloat-abi=hard -mthumb'
        common_flags                    = f' {mcu} --specs=nosys.specs' 
        ...
        linker_script                   = os.path.join( bsp_path, linker_script )
        self._base_release.linkflags    = f' {common_flags} -fno-rtti -fno-exceptions -fno-unwind-tables -fno-use-cxa-atexit -Wl,--script={linker_script}  -Wl,-Map={exename}.map -Wl,--gc-sections -static' 
        self._base_release.linklibs     = ' -Wl,--start-group -lstdc++ -lm -lc -lsupc++ -Wl,--end-group'                                        
        
        # For Reset ISR
        self._base_release.firstobjs    = f'_BUILT_DIR_.{bsp_root}'
        ...
        self._debug_release.cflags     = r'-g3 -DDEBUG -gdwarf-2'
